(()=>{
const VERSION='trello-2026-06-24-pro-2',M='trello_boards';
...
let sb,boards=[],b,drag,q='',showArchived=false,activeCard='',view='board',filterMember='',filterLabel='',toastTimer;

const L=t=>({id:id(),t,c:[]});
const C=t=>({id:id(),t,d:'',due:'',members:[],labels:[],check:[],comments:[],activity:[],createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()});
const empty=(title='PM ERP Trello')=>({title,members:['Leo','PM','設計','業務'],labels:[{id:id(),n:'急件',c:'#ef4444'},{id:id(),n:'客戶',c:'#3b82f6'},{id:id(),n:'財務',c:'#22c55e'},{id:id(),n:'設計',c:'#8b5cf6'}],templates:[{id:id(),n:'客戶需求卡',d:'需求摘要：\n交付物：\n注意事項：',check:['確認需求','估工時','回覆客戶']},{id:id(),n:'設計任務卡',d:'設計方向：\n參考資料：\n輸出規格：',check:['收集參考','初稿','內部審核','交付']}],lists:[L('待辦'),L('進行中'),L('待確認'),L('完成')],archived:[],activity:[]});
...
async function load(){
  ...
  boards=(r.data||[]).map(x=>normalize({id:x.id,...x.data}));
...
function normalize(x){
  ...
  x.members=x.members||['Leo','PM'];
  x.labels=x.labels||empty().labels;
  x.templates=x.templates||[];
  x.lists=x.lists||[];
  x.archived=x.archived||[];
  x.activity=x.activity||[];
  x.lists.forEach(l=>{l.c=l.c||[];l.c.forEach(nc)});
  x.archived.forEach(nc);
  return x;
}
function nc(c){c.members=c.members||[];c.labels=c.labels||[];c.check=c.check||[];c.comments=c.comments||[];c.activity=c.activity||[];return c}
async function save(redraw=true,msg=''){
  b.updatedAt=new Date().toISOString();
  await sb.from('erp_records').upsert({id:b.id,module:M,title:b.title,data:b});
  if(msg)logBoard(msg);
  if(redraw)draw();
}
function logBoard(t){
  b.activity=b.activity||[];
  b.activity.unshift({id:id(),t,at:new Date().toISOString(),by:'我'});
  b.activity=b.activity.slice(0,100);
}
function logCard(c,t){
  c.activity=c.activity||[];
  c.activity.unshift({id:id(),t,at:new Date().toISOString(),by:'我'});
...
function match(c){
  let text=JSON.stringify(c).toLowerCase();
  return (!q||text.includes(q.toLowerCase()))&&(!filterMember||c.members.includes(filterMember))&&(!filterLabel||c.labels.includes(filterLabel));
}
function card(c,l,i=0){
  ...
  return`<article class=card draggable=${l==='archived'?'false':'true'} data-c=${c.id} data-f=${l}>
    ...
    <div class=meta>${c.d?'<span class=badge>📝</span>':''}${c.due?'<span class="badge '+(over?'overdue':'')+'">📅 '+h(c.due)+'</span>':''}${total?'<span class=badge>☑ '+done+'/'+total+'</span>':''}${c.comments.length?'<span class=badge>💬 '+c.comments.length+'</span>':''}<span class=avatars>${(c.members||[]).map(a=>'<span class=avatar>'+h(a[0]||'?')+'</span>').join('')}</span></div>
...
function tableView(){
  let all=allCards().filter(x=>match(x.c));
  return`<main class=board><section class=table-panel><h2>卡片表格</h2><table><thead><tr><th>標題</th><th>列表</th><th>到期日</th><th>成員</th><th>標籤</th><th>檢查</th></tr></thead><tbody>${all.map(({l,c})=>`<tr data-row="${c.id}"><td><b>${h(c.t)}</b><p>${h(c.d||'')}</p></td><td>${h(l.t)}</td><td>${h(c.due||'')}</td><td>${h((c.members||[]).join(', '))}</td><td>${(c.labels||[]).map(x=>{let y=b.labels.find(z=>z.id===x);return h(y?.n||'')}).join(', ')}</td><td>${(c.check||[]).filter(x=>x.done).length}/${(c.check||[]).length}</td></tr>`).join('')}</tbody></table></section>${activityPanel()}</main>`;
}
function calendarView(){
  let dated=allCards().filter(x=>x.c.due&&match(x.c)).sort((a,b)=>a.c.due.localeCompare(b.c.due));
  return`<main class=board><section class=calendar-panel><h2>到期日月曆</h2>${dated.map(({l,c})=>`<article class="calendar-card ${c.due<today()?'late':''}" data-row="${c.id}"><b>${h(c.due)}</b><span>${h(c.t)}</span><small>${h(l.t)}｜${h((c.members||[]).join(', '))}</small></article>`).join('')||'<div class=empty>沒有設定到期日的卡片</div>'}</section>${activityPanel()}</main>`;
}
function activityPanel(){
  return`<aside class=activity-panel><h3>活動紀錄</h3>${(b.activity||[]).slice(0,18).map(a=>`<p><b>${h(a.by)}</b> ${h(a.t)}<small>${h(new Date(a.at).toLocaleString('zh-TW'))}</small></p>`).join('')||'<div class=empty>尚無活動</div>'}</aside>`;
}
...
async function copyBoard(){let t=prompt('複製看板名稱',b.title+' 複製');if(!t)return;let clone=JSON.parse(JSON.stringify(b));clone.id=id();clone.title=t;clone.activity=[];boards.unshift(clone);b=clone;localStorage.setItem('pm_erp_active_board_id',b.id);await save(true,'複製看板')}
